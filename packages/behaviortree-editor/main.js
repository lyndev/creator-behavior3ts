  ...
  messages: {
    ...
    let modelAsObj =  arg;
      ...
      let nodeDataArray = modelAsObj.nodeDataArray;
       ...
       let finalString = "new B3Sequence(##key" + rootNode.key +"##)";
    ...
    for(let node of nodeDataArray){
      ...
      if(node.name != node.type || node.type == 'Action' || node.type == 'Condition' || node.type == 'Composite' || node.type == 'Decorator'){
            ...
            customNodeString += "let  "+node.name+" = B3Class(B3"+node.type+");\n" + 
    ...
    customNodeString + 
    "let tree = new B3BehaviorTree();\n"+
    "tree.root = " + finalString.replace(/"/g,"") + ";\n"+
    "this.tree = tree;\n"+
    "this.blackboard = new B3Blackboard();\n" + 
    "this.b3 = b3;\n" + 
    "},\n" +
